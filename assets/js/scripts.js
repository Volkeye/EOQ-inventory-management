...
// File that creates the reorder details search table
reorderDetailsSearchTableCreatorFile = 'model/reorder/ReorderDetailsSearchTableCreator.php';
...
// File that creates the reorder reports search table
reorderReportsSearchTableCreatorFile = 'model/reorder/ReorderReportsSearchTableCreator.php';
...
purchaseLastInsertedIDFile = 'model/purchase/populateLastPurchaseID.php';

// File that returns the last inserted saleID
saleLastInsertedIDFile = 'model/sale/populateLastSaleID.php';
...
// File that returns the last inserted reorderID
reorderLastInsertedIDFile = 'model/reorder/populateLastReorderID.php';
...
// File that returns reorderIDs
showReorderIDSuggestionsFile = 'model/reoder/showReorderIDs.php';
...
// File that returns itemNumbers for reorder tab
showItemNumberForReorderTabFile = 'model/item/showItemNumberForReorderTab.php';
...
$(document).ready(function(){
	...
	// Listen to update button in reorder details tab
	$('#updateReorderDetailsButton').on('click', function(){
		updateReorder();
	});
	...
	// Listen to delete button in reorder details tab
	$('#deleteReorderButton').on('click', function(){
		// Confirm before deleting
		bootbox.confirm('Are you sure you want to delete?', function(result){
			if(result){
				deleteReorder();
	...
	// Listen to item number text box in reorder details tab
	$('#reorderDetailsItemNumber').keyup(function(){
		showSuggestions('reorderDetailsItemNumber', showItemNumberForreorderTabFile, 'reorderDetailsItemNumberSuggestionsDiv');
	});
	
	// remove the item numbers suggestions dropdown in the purchase details tab
	// when user selects an item from it
	$(document).on('click', '#reorderDetailsItemNumberSuggestionsList li', function(){
		$('#reorderDetailsItemNumber').val($(this).text());
		$('#reorderDetailsItemNumberSuggestionsList').fadeOut();
		
		// Display the item name for the selected item number
		getItemName('reorderDetailsItemNumber', getItemNameFile, 'reorderDetailsItemName');
	...
	// Listen to reorderID text box in reorder details tab
	$('#reorderDetailsreorderID').keyup(function(){
		showSuggestions('reorderDetailsSaleID', showreorderIDSuggestionsFile, 'reorderDetailsreorderIDSuggestionsDiv');
	});
	
	// Remove the reorderID suggestions dropdown in the reorder details tab
	// when user selects an item from it
	$(document).on('click', '#reorderDetailsReorderIDSuggestionsList li', function(){
		$('#reorderDetailsReorderID').val($(this).text());
		$('#reorderDetailsReorderIDSuggestionsList').fadeOut();
		getReorderDetailsToPopulate();
	});
	...
	// Calculate average in reorder tab
	$('#reorderDetailsMaximumSales, #reorderDetailsMinimumSales ').change(function(){
		calculateAverageInReorderTab();
	});
	
	// Calculate Threshold in purchase tab
	$('#reorderDetailsAverageSales, #reorderDetailsDeliveryTime').change(function(){
		calculateThresholdInReorderTab();
	});
	...
	// Load searchable datatables for customer, purchase, item, vendor, sale ,reorder
	...
	searchTableCreator('reorderDetailsTableDiv', reorderDetailsSearchTableCreatorFile, 'reorderDetailsTable');

	
	// Load searchable datatables for customer, purchase, item, vendor, sale ,reorder reports
	...
	reportsTableCreator('reorderReportsTableDiv', reorderReportsSearchTableCreatorFile, 'reorderReportsTable');
...
// Function to create searchable datatables for customer, item, purchase, sale ,reorder
...
// Function to create reports datatables for customer, item, purchase, sale ,reorder
...
// Calculate Total average in reorder details tab
function calculateAverageInReorderTab(){
	var maxSalesRT = $('#reorderDetailsMaximumSales').val();
	var minSalesRT = $('#reorderDetailsMinimumSales').val();
	$('#reorderDetailsAverageSales').val((Number(maxSalesRT) + Number(minSalesRT))/2);
}

// Calculate threshold in reorder details tab
function calculateThresholdInReorderTab(){
	var averageSalesRT = $('#reorderDetailsAverageSales').val();
	var deliveryTimeRT = $('#reorderDetailsDeliveryTime').val();
	$('#reorderDetailsStockThreshold').val(Number(averageSalesRT) * Number(deliveryTimeRT));
}
...
// Function to call the Insertreorder.php script to insert reorder data to db
function addReorder() {
	var reorderDetailsItemNumber = $('#reorderDetailsItemNumber').val();
	var reorderDetailsItemName = $('#reorderDetailsItemName').val();
	var reorderDetailsMaximumSales = $('#reorderDetailsMaximumSales').val();
	var reorderDetailsMinimumSales = $('#reorderDetailsMinimumSales').val();
	var reorderDetailsAverageSales = $('#reorderDetailsAverageSales').val();
	var reorderDetailsDeliveryTime = $('#reorderDetailsDeliveryTime').val();
	var reorderDetailsStockThreshold = $('#reorderDetailsStockThreshold').val();
	
	$.ajax({
		url: 'model/reorder/Insertreorder.php',
		method: 'POST',
		data: {
			reorderDetailsItemNumber:reorderDetailsItemNumber,
			reorderDetailsItemName:reorderDetailsItemName,
			reorderDetailsMaximumSales:reorderDetailsMaximumSales,
			reorderDetailsMinimumSales:reorderDetailsMinimumSales,
			reorderDetailsAverageSales:reorderDetailsAverageSales,
			reorderDetailsDeliveryTime:reorderDetailsDeliveryTime,
			reorderDetailsStockThreshold:reorderDetailsStockThreshold ,
		},
		success: function(data){
			$('#reorderDetailsMessage').fadeIn();
			$('#reorderDetailsMessage').html(data);
		},
		complete: function(){
			populateLastInsertedID(reorderLastInsertedIDFile, 'reorderDetailsReorderID');
			searchTableCreator('reorderDetailsTableDiv', reorderDetailsSearchTableCreatorFile, 'reorderDetailsTable');
			reportsTableCreator('reorderReportsTableDiv', reorderReportsSearchTableCreatorFile, 'reorderReportsTable');
			searchTableCreator('itemDetailsTableDiv', itemDetailsSearchTableCreatorFile, 'itemDetailsTable');
			reportsTableCreator('itemReportsTableDiv', itemReportsSearchTableCreatorFile, 'itemReportsTable');
...
// Function to delete reorder from db
function deleteReorder(){
	// Get the item number entered by the user
	var reorderDetailsItemNumber = $('#reorderDetailsItemNumber').val();
	
	// Call the deleteReorder.php script only if there is a value in the
	// ItemNumber textbox
	if(reorderDetailsItemNumber != ''){
		$.ajax({
			url: 'model/reorder/Deletereorder.php',
			method: 'POST',
			data: {reorderDetailsItemNumber:reorderDetailsItemNumber},
			success: function(data){
				$('#reorderDetailsMessage').fadeIn();
				$('#reorderDetailsMessage').html(data);
			},
...
function getPurchaseDetailsToPopulate(){
	...
	$.ajax({
		...
		complete: function(){
			calculateTotalInPurchaseTab();
			getItemStockToPopulate('purchaseDetailsItemNumber', getItemStockFile, 'purchaseDetailsCurrentStock');
			getItemNameToPopulate('purchaseDetailsItemNumber', getItemStockFile, 'purchaseDetailsItemName');
		}
	});
}
...
// Function to send reorderID so that reorder details can be pulled from db
// to be displayed on reorder details tab
function getReorderDetailsToPopulate(){
	// Get the ReorderID entered in the text box
	var reorderDetailsReorderID = $('#reorderDetailsReorderID').val();
	
	// Call the populateReorderDetails.php script to get item details
	// relevant to the itemNumber which the user entered
	$.ajax({
		url: 'model/reorder/populateReorderDetails.php',
		method: 'POST',
		data: {reorderDetailsReorderID:reorderDetailsReorderID},
		dataType: 'json',
		success: function(data){
			$('#reorderDetailsItemNumber').val(data.itemNumber);
			$('#reorderDetailsItemName').val(data.itemName);
			$('#reorderDetailsMaximumSales').val(data.maxSales);
			$('#reorderDetailsMinimumSales').val(data.minSales);
			$('#reorderDetailsAverageSales').val(data.averageSales);
			$('#reorderDetailsDeliveryTime').val(data.deliveryTime);
			$('#reorderDetailsStockThreshold').val(data.stockThreshold);
		},
		complete: function(){
			calculateAverageInReorderTab();
			getItemNameToPopulate('reorderDetailsItemNumber', getItemNameFile, 'reorderDetailsItemName');
		}
	});
}
...
// Function to call the updateCustomerDetails.php script to UPDATE customer data in db
...
// Function to call the updateReorder.php script to update reorder data to db
function updateReorder() {
	...
	$.ajax({
		url: 'model/reorder/updateReorderDetails.php',
		method: 'POST',
		data: {
			reorderDetailsItemNumber:reorderDetailsItemNumber,
			reorderDetailsItemName:reorderDetailsItemName,
